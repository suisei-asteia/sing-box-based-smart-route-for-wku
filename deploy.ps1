...
.DESCRIPTION
    ...
    脚本运行结束后会停留等待手动关闭，不会闪退。
.NOTES
    需管理员权限（脚本会自动提权）。卸载用 restore.ps1。
    部署前建议先运行 check.ps1 检查环境。
#>
[CmdletBinding()]
param(
...
try {
    ...
    $taskAction = New-ScheduledTaskAction -Execute $exe -Argument "run -D `"$InstallDir`" -c `"$cfgPath`""
    $taskTrigger = New-ScheduledTaskTrigger -AtStartup
    $taskSettings = New-ScheduledTaskSettingsSet -RestartCount 999 -RestartInterval (New-TimeSpan -Minutes 1) -ExecutionTimeLimit ([TimeSpan]::Zero) -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries
    ...
    Write-Host ("任务状态: LastTaskResult={0}  LastRunTime={1}" -f $info.LastTaskResult, $info.LastRunTime)
    Write-Host ''
    ...
    Write-Host '部署失败。请将上面的错误信息截图，用于排查。' -ForegroundColor Yellow
}
